...
当前 tag 已经实现 `ToolRegistry` 和 `echo_tool`，足够解释 model tool call 到 tool result 再回到 provider request 的最小路径。后续已经补上部分 workspace/runtime tools 的 MVP 记录，但完整 tool layer 仍要继续增强。

关键文件：

- `packages/agent-runtime/src/types/tool.type.ts`
- `packages/agent-runtime/src/tools/tool-registry.ts`
- `packages/agent-runtime/src/tools/echo-tool.ts`
- `packages/agent-runtime/tests/tool-registry.test.ts`
...
- `echo_tool` 是教学 tool。
- 真实 file / shell / git tools 仍是 MVP。
- 还没有完整 permission check。
- `ToolResult` 仍需要升级到完整 envelope。
- 还没有完整 token budget、raw artifact、redaction 和 context guard。
...
Lesson 1.3 先做 tool pipeline，不被 HookRunner 阻塞：
...
6. Lesson 1.7 再实现 `HookRunner` 调用这些 payload。
